...
function trimLogFile(logPath: string, maxSize: number) {
  try {
    ...
    // ignore logging issues in dev tooling
  }
}
  ...
  return {
    ...
    transformIndexHtml(html) {
      if (
        ...
        !isEnabled(process.env.ENABLE_MANUS_DEBUG_COLLECTOR, false)
      ) {
        return html;
      }
    ...
    configureServer(server: ViteDevServer) {
      if (!isEnabled(process.env.ENABLE_MANUS_DEBUG_COLLECTOR, false)) {
        return;
      }
...
export function getManusVitePlugins(): Plugin[] {
  const plugins: Plugin[] = [];

  if (isEnabled(process.env.ENABLE_MANUS_RUNTIME, true)) {
